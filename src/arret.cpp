...
void Arret::VehiculeEnter(boost::shared_ptr<Vehicule> pVehicle, VoieMicro * pDestinationEnterLane, double dbInstDestinationReached, double dbInstant, double dbTimeStep, bool bForcedOutside)
{
    ...
    // on ne remplit pas les map en mode non SymuMaster (pas utilis� et m�chanisme de nettoyage de la map int�gr�e aux fonctions de calcul qui l'utilisent)
    if (m_pNetwork->IsSymuMasterMode())
    {
        // gestion du dernier arr�t de la ligne (le v�hicule disparait et on ne passe pas dans GetStopDuration...)
...
void Arret::VehiculeExit(boost::shared_ptr<Vehicule> pVehicle)
{
    ...
    if (m_pNetwork->IsSymuMasterMode())
    {
        ...
        {
            // gestion de la mont�e au premier arr�t (on ne passe pas dans GetStopDuration dans ce cas ...)
            LoadUsageParameters & loadParams = pVehicle->GetFleetParameters()->GetUsageParameters().GetLoadUsageParameters();
        ...
        {
            //rmq. : un bus peut ne pas �tre pr�sent pour l'instant dans la map s'il a �t� cr�� � l'arr�t :
...
// ajoute la ligne � la liste des lignes conern�es par l'arret
void Arret::AjoutLigne(Trip* ligne, bool bDureeArretDynamique, bool bPoisson, double dbRatioDescente, bool bBinomial, int nStockInit, RepartitionTypeVehicule * pRepTypesVeh, int num_voie)
{
    ...
    // Construction du num�ro de la voie de l'arr�t pour chacune des lignes. On prend par d�faut la voie
    // la plus � droite qui n'est pas interdite.
...
//---------------------------------------------------------------------------
// Fonctions de renvoi de donn�es de l'arret
//--------------------------------------------------------------------------
...
// retourne le num�ro de la voie sur lequel se trouve l'arr�t pour la ligne consid�r�e
...
double Arret::GetStopDuration(boost::shared_ptr<Vehicule> pVehicle, bool bIsRealStop)
{
    ...
    if(!isDureeArretDynamique(pLigne) && !m_pNetwork->IsSymuMasterMode())
    {
        ...
        // Si on a une dur�e d'arr�t d�finie pour cet arr�t, on l'utilise � la place
    ...
    {
        // Cas du calcul dynamique de la dur�e de l'arr�t
        LoadUsageParameters & loadParams = pVehicle->GetFleetParameters()->GetUsageParameters().GetLoadUsageParameters();
        ...
        else
        {
            ...
            if (!isPoisson(pLigne))
            {
                // Mode d�terministe. On arrondi pour avoir un nombre de passagers entiers
        ...
        {
            // l'arr�t est vide de pi�tons puisqu'ils ont tous pu monter
            nbPietonsRestant = 0;
        }
        
        // Calcul du temps d'arr�t
        ...
        if(bIsRealStop)
        {
            ...
            {
                // Application des modifications li�es � un arr�t concr�tis�
        ...
        // On s'arr�te au moins un pas de temps
...
TripNodeSnapshot * Arret::TakeSnapshot()
{
    ...
    // par rapport � la classe m�re, on sauvegarde aussi les derniers instants de passages
...
void Arret::Restore(Reseau * pNetwork, TripNodeSnapshot * backup)
{
    ...
    // par rapport � la classe m�re, on restore aussi les derniers instants de passages
