...
#include <SymuCore/Graph/Model/PublicTransport/WalkingPattern.h>
#include <SymuCore/Graph/Model/PatternsSwitch.h>
#include <SymuCore/Graph/Model/NullPenalty.h>
#include <SymuCore/Graph/Model/PublicTransport/PublicTransportLine.h>
#include <SymuCore/Demand/MacroType.h>
#include <SymuCore/Demand/Path.h>
...
{
    // 1 - Recup�ration des PublicTransportFleet
    ...
    for (size_t iTripNode = 0; iTripNode < m_pPublicTransportFleet->GetTripNodes().size(); iTripNode++)
    {
        ...
        // Un arret est forcement rattach� a un tuyau
    ...
    // 3 - cr�ation des PublicTransportLines et des Links entre les arr�ts d'une m�me ligne
    ::Trip * pTrip;
    Link * pLink;
    ...
    for (size_t iTrip = 0; iTrip < m_pPublicTransportFleet->GetTrips().size(); iTrip++)
    {
        ...
        //Cr�ation et ajout du PublicTransportLine
        ...
        //identification des diff�rents couples d'arr�ts
        ...
        for(size_t i = 0; i < pTrip->GetLegs().size(); i++)
        {
            ...
            {
                // On cr�e un seul Link m�me si plusieurs trip relient le m�me couple noeud amont + noeud aval
                ...
                {
                    // pas de doublon pour le moment : on cr�e le link
    ...
    // 4 - cr�ation des links entre lignes de bus
    BuildLinksBetweenLines(pPublicTransportLayer);

    assert(m_pNetwork->IsWithPublicTransportGraph() || m_pNetwork->IsSymuMasterMode());

    // Pour le SG (donc sans SymuMaster), on ne g�re pas les notions de zones ni de park relai (en tout cas pas 
    // comme �a pour les parcs relais : on d�compose les calculs d'itin�raires en une phase VL et une phase TEC)
    if (m_pNetwork->IsSymuMasterMode())
    {
        // 5 - cr�ation des Links de marche en zone / plaque
        BuildLinksWithZones(pPublicTransportLayer, pDrivingGraphBuilder);

        // 6 - cr�ation des Links de marche entre parcs relais et arr�ts associ�s
    ...
    // mais suite au besoin Mostafa, on fait ces deux �tapes dans tous les cas � pr�sent.

    // 7 - cr�ation des Links de marche enter origine potentielle et arr�t
    BuildLinksWithOrigins(pPublicTransportLayer, pDrivingGraphBuilder);

    // 8 - cr�ation des Links de marche entre arret et destination potentielle
    BuildLinksWithDestinations(pPublicTransportLayer, pDrivingGraphBuilder);

    // 9 - cr�ation des patterns et penalties
...
void SymuCorePublicTransportGraphBuilder::BuildLinksBetweenLines(SymuCore::Graph * pPublicTransportLayer)
{
    for (std::map<Arret*, Node*, LessConnexionPtr<Arret> >::const_iterator iterStop1 = m_mapArretsToNode.begin(); iterStop1 != m_mapArretsToNode.end(); ++iterStop1)
    {
        for (std::map<Arret*, Node*, LessConnexionPtr<Arret> >::const_iterator iterStop2 = m_mapArretsToNode.begin(); iterStop2 != m_mapArretsToNode.end(); ++iterStop2)
        {
            ...
            {
                // On v�rifie que les deux arr�ts ne partagent pas une ligne
                ...
                {
                    // Si la distance est inf�rieure au radius max de marche, cr�ation d'un link
...
void SymuCorePublicTransportGraphBuilder::BuildLinksWithZones(SymuCore::Graph * pPublicTransportLayer, SymuCoreDrivingGraphBuilder * pDrivingGraphBuilder)
{
    ...
    for (std::map<ZoneDeTerminaison*, SymuCore::Node*, LessConnexionPtr<ZoneDeTerminaison> >::const_iterator iterZones = mapZones.begin(); iterZones != mapZones.end(); ++iterZones)
    {
        ...
        // identification de l'ensemble des arr�ts de la zone :
        for (std::map<Arret*, Node*, LessConnexionPtr<Arret> >::const_iterator iterStop = m_mapArretsToNode.begin(); iterStop != m_mapArretsToNode.end(); ++iterStop)
        {
            ...
            {
                // l'arr�t est en zone, cr�ation des links correspondants, quel que soit la distance malgr� le radius de marche d�fini.
				if (bZoneOrigin)
                {
    ...
    for (std::map<CPlaque*, SymuCore::Node*, LessConnexionPtr<CPlaque> >::const_iterator iterPlaques = mapPlaques.begin(); iterPlaques != mapPlaques.end(); ++iterPlaques)
    {
        ...
        // Constitution de la liste des arr�ts de la zone :
        ZoneDeTerminaison * pParentZone = pPlaque->GetParentZone();

        // identification de l'ensemble des arr�ts de la zone et calcul de la distance � la plaque :
        ...
        for (std::map<Arret*, Node*, LessConnexionPtr<Arret> >::const_iterator iterStop = m_mapArretsToNode.begin(); iterStop != m_mapArretsToNode.end(); ++iterStop)
        {
            ...
            {
                // S'il n'est pas dans la plaque, on regarde quand m�me s'il est dans la zone en calculant la distance
        ...
        // du coup, on conserve tous les arr�ts de la plaque plus jusqu'� N plus proches dans la zone:
    ...
    for (std::map<Arret*, Node*, LessConnexionPtr<Arret> >::const_iterator iterStop = m_mapArretsToNode.begin(); iterStop != m_mapArretsToNode.end(); ++iterStop)
    {
        for (size_t iNode = 0; iNode < listNodes.size(); iNode++)
        {
            ...
            if (dbDistance <= m_pNetwork->GetMaxInitialWalkRadius())
            {
                ...
                // on �limine les sorties en v�rifiant qu'on a au moins un tron�on aval
...
{
    // 1 - cr�ation des patterns
    BuildPatterns(pPublicTransportLayer);

    // 2 - cr�ation des penalties
...
{
    // 1 - Cr�ation de patterns en bus le long des lignes
    Link * pLink;
    ...
    for (itMap = m_mapLinksToTuyaux.begin(); itMap != m_mapLinksToTuyaux.end(); ++itMap)
    {
        ...
        // Pour chaque ligne passant par ce Link, cr�ation des patterns correspondants :
        ...
        // Un pattern pour la marche � pieds si la distance est bonne
    ...
    // 2 - cr�ation de patterns de marche
...
{
    // 1 - cr�ation des p�nalit�s aux arr�ts
    ...
    for (iterNode = m_mapArretsToNode.begin(); iterNode != m_mapArretsToNode.end(); ++iterNode)
    {
		...
		for (size_t iUpstreamLink = 0; iUpstreamLink < pNode->getUpstreamLinks().size(); iUpstreamLink++)
		{
            ...
            // pas encore de liens intercouches � ce niveau
    ...
    // 2 - cr�ation de p�nalit�s aux parcs relais
            ...
            for (size_t iUpstreamPattern = 0; iUpstreamPattern < pUpstreamLink->getListPatterns().size(); iUpstreamPattern++)
            {
                ...
                for (size_t iDownstreamLink = 0; iDownstreamLink < pNode->getDownstreamLinks().size(); iDownstreamLink++)
                {
                    ...
                    for (size_t iDownstreamPattern = 0; iDownstreamPattern < pDownstreamLink->getListPatterns().size(); iDownstreamPattern++)
                    {
                        ...
                        // Pas de cr�ation de p�nalit� dans les autres cas (on peut pas arriver en conduisant et repartir en conduisant, idem pour la marche � pieds
                    }
                }
            }
        }
